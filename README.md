...
**Master branch:** 13 keep experiments tracked in git commits (baseline Jetson port)  
**apr4 branch:** 20 keep experiments + final results
...
See [results.tsv](https://github.com/radozaprazny/autoresearch-jetson/blob/autoresearch/apr4/results.tsv) (apr4 branch) for complete experiment log.
...
- Minimal changes from upstream for ARM64 compatibility  
- Keep experiments tracked via git commits (no `results.tsv` committed)

**`autoresearch/apr4`** — With workflow improvements  
- All 6 integrity checks enabled
- Final `results.tsv` committed ([view here](https://github.com/radozaprazny/autoresearch-jetson/blob/autoresearch/apr4/results.tsv))
- Tagged as `jetson-apr4-final`
...
# 2. Install dependencies (JetPack PyTorch wheel uses non-standard filename)
UV_SKIP_WHEEL_FILENAME_CHECK=1 uv sync

# 3. Download data and train tokenizer (one-time, ~2 min)
uv run prepare.py

# 4. Run single experiment (~5 min)
uv run train.py
```

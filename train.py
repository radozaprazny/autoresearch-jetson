...
# Use PyTorch built-in SDPA — works on all CUDA GPUs without extra kernels
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        ...
        # SDPA instead of Flash Attention 3 (ARM64 compatible)
        # (B, T, H, D) -> (B, H, T, D)
        q = q.transpose(1, 2)
        k = k.transpose(1, 2)
        v = v.transpose(1, 2)
...
# Optimization
TOTAL_BATCH_SIZE = 2**14 # ~16K tokens per optimizer step — reduced for Jetson to get more steps
EMBEDDING_LR = 0.58     # learning rate for token embeddings (Adam)
UNEMBEDDING_LR = 0.0038 # learning rate for lm_head (Adam)
MATRIX_LR = 0.0285      # learning rate for matrix parameters (Muon)
SCALAR_LR = 0.5         # learning rate for per-layer scalars (Adam)
WEIGHT_DECAY = 0.10     # cautious weight decay for Muon
